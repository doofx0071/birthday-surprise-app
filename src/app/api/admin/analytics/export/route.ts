...
export async function POST(request: NextRequest) {
  try {
    ...
    if (format === 'csv') {
      ...
      const withReminders = messages?.filter(m => m.wants_reminders).length || 0
      ...
      // Media statistics
      const imageFiles = mediaFiles?.filter(f => f.file_type === 'image' || f.file_type?.startsWith('image/')).length || 0
      const videoFiles = mediaFiles?.filter(f => f.file_type === 'video' || f.file_type?.startsWith('video/')).length || 0
...
Media Attachment Rate: ${totalMessages > 0 ? Math.round((mediaFiles?.filter(f => messages?.some(m => m.id === f.message_id)).length || 0) / totalMessages * 100) : 0}%
